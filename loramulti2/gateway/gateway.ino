...
String readingID;
int node=1;
unsigned long timerDelay1 = 20000;
unsigned long timerDelay2 = 25000;
...
void loop(){
  if ((millis() - lastTime1) > timerDelay1 && node==1) {
    send_request(node);
    if((millis() - lastTime1)>timerDelay1+3000){
      node=2;
      lastTime1=millis();
    }
  }
  if ((millis() - lastTime1) > timerDelay1 && node==2) {
    send_request(node);
    if((millis() - lastTime1)>timerDelay1+3000){
      node=1;
      lastTime1=millis();
